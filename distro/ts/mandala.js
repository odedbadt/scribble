...
function rotate(v, center, a, mirror) {
    const v2 = {
...
export class mandala extends ClickAndDragTool {
    ...
    editing_action(to) {
        ...
        const r = this.app.settings.line_width / 2;
        const dist = Math.sqrt(dist2([this.center.x, this.center.y], [to.x, to.y]));
        this.extend_canvas({
        ...
        Array.from([true, false]).forEach((mirror) => {
            this._angles.forEach((angle) => {
                const rotated_to = rotate(to, this.center, angle, mirror);
                ...
                if (this._recorded_to) {
                    const rotated_recorded = rotate(this._recorded_to, this.center, angle, mirror);
                    this.context.beginPath();
                    this.staging_context.beginPath();
